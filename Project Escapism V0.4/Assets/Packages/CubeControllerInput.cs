...
public class CubeControllerInput : BaseInputModule
{
	public static CubeControllerInput Instance;
	...
	protected override void Start()
	{
		...
		if (Initialized == false)
		{
			...
			/*here should be the left and right cubes*/Pointers = new GameObject[] { hand };
			//ControllerDevices = new SteamVR_Controller.Device[Controllers.Length];
			Cursors = new RectTransform[Pointers.Length];
	...
	{
		ControllerCamera.transform.position = Pointers[index].transform.position;
		ControllerCamera.transform.forward = Pointers[index].transform.forward;
	}
	/*
	...
	*/
	...
	public override void Process()
	{
		...
		for (int index = 0; index < Cursors.Length; index++)
		{
			if (Pointers[index].gameObject.activeInHierarchy == false)
			{
