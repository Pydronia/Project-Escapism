			...
			if (alreadyInst == false) {
				if (hit) {
					if (hitPos.collider.tag == "environment" || hitPos.collider.tag == "object") {
						...
						manipObj.tag = "manip";
						alreadyInst = true;
					} //TODO: add else here to prevent placing of objects on things other than just objects and evnironment.
				}
			} else if (!hit) {//if it's not hitting anything, save rotation, destroy it and reinstantiate it when it hits something again
